...
function addOverlay() {
	...
	$("#ovcol-" + overlayId).spectrum("set", defaultOverlayColor);
...
function getSliderString(overlay, id) {
	...
	for (var i = 0; i < overlay.sliders.length; i++) {
		...
		string += "<label for=\"slider-" + name + "-" + i + "\">" + label + "</label>" +
		          "<input type=\"range\" name=\"" + name + "\" id=\"" + name + "\" data-highlight=\"true\" min=\"" + (useX ? -maxX : -maxY) +  "\" max=\"" + (useX ? maxX : maxY) + "\" value=\"" + overlay.sliders[i][2] + "\" step=\".01\" ";
		if (useX) {
			string += "use-x ";
		} else {
			string += "use-y ";
